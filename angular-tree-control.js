        ...
        .directive('treecontrol', ['$compile', function ($compile) {
            ...
            return {
                ...
                controller: ['$scope', '$templateCache', '$interpolate', 'treeConfig', function ($scope, $templateCache, $interpolate, treeConfig) {
                    ...
                    $scope.selectNodeLabel = function (selectedNode, evt) {
                        var transcludedScope = this;
                        if (!$scope.options.isLeaf(selectedNode, $scope) && (!$scope.options.dirSelectable || !$scope.options.isSelectable(selectedNode))) {
                            ...
                            if ($scope.options.multiSelection) {
                                ...
                                if (evt.ctrlKey) {
                                    ...
                                    let lastSelected = $scope.selectedNodes[$scope.selectedNodes.length - 1];
                                    let indexLast = parentNode.children.findIndex(item=>item.GwID === lastSelected.GwID);
                                    let indexSelected = parentNode.children.findIndex(item=>item.GwID === selectedNode.GwID);
                                    if( indexLast >= 0 && indexSelected >= 0 ){
                                        for (let i = Math.min(indexLast, indexSelected); i <= Math.max(indexLast, indexSelected); i++){
                                            let selectedNode = parentNode.children[i];
                                            var pos = -1;
                                            for (var j = 0; j < $scope.selectedNodes.length; j++) {
                                                if ($scope.options.equality(selectedNode, $scope.selectedNodes[j], $scope)) {
                                                    pos = j;
                                                    break;
                                                }
                                            }
                    ...
                    if (!template) {
                        template =
                            ...
                            '<div class="tree-label {{options.labelClass}}" ng-class="[selectedClass(), unselectableClass()]" ng-click="selectNodeLabel(node,$event)" ' + rcLabel + ctxMenuId + ' tree-transclude></div>' +
